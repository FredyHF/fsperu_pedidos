...
class pedido_cliente extends fs_model {
   ...
   {
      parent::__construct('pedidoscli', 'plugins/fsperu_pedidos/');
      if ($p)
      {
   ...
   public function all_contados($offset = 0, $order = 'DESC')
   {
      $preclist = array();
      
      $pedidos = $this->db->select_limit("SELECT * FROM " . $this->table_name .
              " WHERE codpago='CONT' ORDER BY fecha " . 
			  $order . ", codigo " . $order, FS_ITEM_LIMIT, $offset);
      if ($pedidos)
      {
         foreach ($pedidos as $p)
            $preclist[] = new pedido_cliente($p);
   ...
   public function all_cancelados($offset = 0, $order = 'DESC')
   {
      $preclist = array();
      
      $pedidos = $this->db->select_limit("SELECT * FROM " . $this->table_name .
              " WHERE codpago LIKE '%CRED%' and status=11 ORDER BY fecha " . 
			  $order . ", codigo " . $order, FS_ITEM_LIMIT, $offset);
      if ($pedidos)
      {
         foreach ($pedidos as $p)
   ...
   public function all_anulados($offset = 0, $order = 'DESC')
   {
      $preclist = array();
      
      $pedidos = $this->db->select_limit("SELECT * FROM " . $this->table_name .
              " WHERE codpago LIKE '%CRED%' and status=10 ORDER BY fecha " . $order . ", codigo " . $order, FS_ITEM_LIMIT, $offset);
      if ($pedidos)
      {
         foreach ($pedidos as $p)
            $preclist[] = new pedido_cliente($p);
   ...
   public function all_creditos($offset = 0, $order = 'DESC')
   {
      $preclist = array();
      
      $pedidos = $this->db->select_limit("SELECT * FROM " . $this->table_name .
              " WHERE codpago LIKE '%CRED%' and status=1 ORDER BY fecha " . $order . ", codigo " . $order, FS_ITEM_LIMIT, $offset);
      if ($pedidos)
      {
         foreach ($pedidos as $p)
            $preclist[] = new pedido_cliente($p);
   ...
   public function search($query, $offset = 0)
   {
      ...
      else
      {
         $consulta .= "lower(codigo) LIKE '%" . $query . "%' OR lower(numero2) LIKE '%" . $query . "%' "
				 ...
				 . "OR lower(nombrecliente) LIKE '%" . str_replace(' ', '%', $query) . "%'";
      }
